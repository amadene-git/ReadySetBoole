...
public:
  ...
  BytesSet(const std::vector<int>& set) : _set(set) {
    ...
    _bytes = std::vector<uint8_t>((set.size() / 8) + !_isLastBytesComplete,
                                  uint8_t{0});
  };
  BytesSet(const std::vector<int>& globalSet, const std::vector<int>& subSet)
      : _set(globalSet) {
    _isLastBytesComplete = (_set.size() % 8 == 0);
  ...
  void setBit(bool value, uint64_t index) {
    if (value == true) {
  ...
  bool getBit(uint64_t index) {
    return _bytes[index / 8] & (1 << (index % 8));
  }
